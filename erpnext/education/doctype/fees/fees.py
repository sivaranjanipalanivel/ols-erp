...
# @frappe.whitelist()
# def get_student_scholarship(student,student_name):
# 	# enrollment = frappe.get_list("Student Scholarship", fields=["student", "student_name","eligible_scholarship","marks_obtained"])
# 	# enrollment = frappe.db.sql("SELECT student, student_name, eligible_scholarship, marks_obtained from `tabStudent Scholarship` where student= %s", student)
# 	return frappe.get_list("Student Scholarship", filters={"student": student}, fields=["student", "student_name","eligible_scholarship","marks_obtained"])
	# if enrollment:
	# 	student_list = []
	# for s in enrollment:
	# 	if s.eligible_scholarship:
	# 		s.update({"is_scholarship_eligible": 1})
	# 	# else:
	# 	# 	s.update({"is_scholarship_eligible": 0})
	...
	def calculate_total(self):
		...
		for d in self.components:
			...
			if self.discount_amount:
				self.total=int(self.grand_total)-int(self.discount_amount)
				self.outstanding_amount = self.total
				self.grand_total_in_words = money_in_words(self.total)
...
def get_fee_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by="modified"):
	...
	if student:
		return frappe. db.sql('''select name, program, due_date, paid_amount, outstanding_amount, total from `tabFees`
