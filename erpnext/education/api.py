...
def get_coupon_details(coupon_code):
	coupon_list = frappe.get_list("Coupons", fields = ['name, discount, price'], filters = {
		"name": coupon_code
	})
	return coupon_list
...
def post_assignment_details(assignment_name,description,due_date,student_group):  
	# if frappe.session.user == "Guest":

		Ass= frappe.get_doc({
			"doctype": "Assignments",
			"name1":assignment_name ,
...
@frappe.whitelist(allow_guest=True)
def post_discussion(disscussion_name,description,is_visible,student_group):
	# if frappe.session.user == "Guest":

		Dis= frappe.get_doc({
			"doctype": "Discussion",
			"name1":disscussion_name ,
			"description": description,
...
@frappe.whitelist(allow_guest=True)
def post_attendance(employee,employee_name,status,leave_type,attendance_date,company):
	Dis= frappe.get_doc({
		"doctype": "Attendance",
		"employee":employee ,
...
@frappe.whitelist(allow_guest=True)
def getemployeeattendance(employee,attendance_date):
	Dis= frappe.db.get_all('Attendance', fields=['name','employee','attendance_date'],filters={'employee':employee,'attendance_date':attendance_date},limit_page_length=50) 	
	return Dis

@frappe.whitelist(allow_guest=True)
def getemployeelist():
	Dis= frappe.db.get_all('Employee', fields=['employee','employee_name','company','user_id','date_of_joining','date_of_birth','gender','status','employment_type','cell_number','personal_email','permanent_address','current_address','marital_status','roll_no']) 	
	return Dis

@frappe.whitelist(allow_guest=True)
def post_employee(employee_name,company,user_id,date_of_joining,date_of_birth,gender,status,employment_type,cell_number,personal_email,current_address,marital_status,leave_type,roll_no):
	Dis= frappe.get_doc({
		"doctype": "Employee",
		"employee_name": employee_name,
		"company":company,
...
@frappe.whitelist(allow_guest=True)
def get_employee(employee,roll_no):
	Dis= frappe.db.get_all('Employee', fields=['employee','employee_name','company','user_id','date_of_joining','date_of_birth','gender','status','employment_type','cell_number','personal_email','permanent_address','current_address','marital_status','roll_no'],filters={'employee':employee,'roll_no':roll_no},limit_page_length=1) 	
	return Dis

@frappe.whitelist(allow_guest=True)
def get_olddecrypted_password():
	# auth=Dis= frappe.db.get_all('__Auth', fields=['roll_no'],filters={},limit_page_length=1) 	
	auth = frappe.db.sql('''select password from `__Auth` where doctype="User"''')
	return auth
