frappe.ui.form.on("Employee Attendance Tool", {
	...
	leave_save: function(frm) {
		employee=[]
		employee_name=[]
		leave_type=[]
		for(var i=0;i<frm.doc.employees.length;i++) {
			employee= frm.doc.employees[i].employee;
			employee_name= frm.doc.employees[i].employee_name;
			leave_type= frm.doc.employees[i].leave_type;
					
				frappe.call({
					method: "erpnext.hr.doctype.employee_attendance_tool.employee_attendance_tool.mark_employee_leave",
					args:{
						"employee":employee,
...
// frappe.ui.form.on("Leave Type Tool", "refresh", function(frm, doctype, name){
...
erpnext.EmployeeSelector = Class.extend({
	...
	make: function(frm, employee) {
		...
		var mark_employee_toolbar = $('<div class="col-sm-12 bottom-toolbar">\
			...
			<button class="btn btn-default btn-mark-onleave btn-xs"></button>\
		...
		mark_employee_toolbar.find(".btn-mark-present")
        	...
        	mark_employee_toolbar.find(".btn-mark-onleave")
			.html(__('Mark On Leave'))
			.on("click", function() {
				var employee_onleave = [];
				$(me.wrapper).find('input[type="checkbox"]').each(function(i, check) {
					if($(check).is(":checked")) {
						employee_onleave.push(employee[i]);
						
					}
					
					
				});
					$.each(employee_onleave, function(i, d) {
						var row = frappe.model.add_child(frm.doc, "Leave Type Tool", "employees");
...
frappe.ui.form.on("Employee Attendance Tool", {
    employees: function(frm, cdt, cdn) {
        var d = locals[cdt][cdn];
        // frappe.model.set_value(d.doctype, d.name, "valor", d.denominacion * d.cantidad);
        
        frm.doc.employees.forEach(function(d) { var total = d.employee; });
        console.log(total)
...
frappe.ui.form.on("Leave Type Tool", "employees", function(frm, cdt, cdn) {

   var material_details = frm.doc.employee
